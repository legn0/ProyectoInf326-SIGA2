...
const EnrollmentAlumnos = () => {
    ...
    const [ramos, setRamos] = useState([
        ...
        { id: 6, nombre: "Historia", codigo: "HIS105", creditos: 4, inscritos: "23/35", estado: "Pendiente",paralelos: [{ id: 11, numero: 200 }, { id: 12, numero: 201 }] },
        { id: 7, nombre: "Historia", codigo: "HIS105", creditos: 4, inscritos: "23/35", estado: "Pendiente",paralelos: [{ id: 13, numero: 200 }, { id: 14, numero: 201 }] },
    ]);
    ...
    const handleEliminar = (cursoId) => {
        setSelectedParallels((prev) => prev.filter((item) => item.cursoId !== cursoId));
    };
        ...
        <Box>
            ...
            <Box mt={8} p={5} bg="white" width="100%">
                ...
                <Stack separator={<StackSeparator />} align="stretch" spacing={4} borderWidth={3} borderRadius="md" p={4} shadow="md" maxHeight="35vh" overflowY="auto">
                    {selectedParallels.map((inscrito) => {
                        const ramo = ramos.find((ramo) => ramo.id === inscrito.cursoId);
                        return (
                            <Box key={inscrito.cursoId}>
                                <HStack spacing={4} justify="space-between" align="center">
                                    ...
                                    <Button variant="outline" color="gray.800" _hover={{ backgroundColor: "orange.300", color: "white" }} onClick={() => handleSelectParallel(inscrito.cursoId, inscrito.paraleloId)}>
                                        Cambiar Paralelo
                                    </Button>
